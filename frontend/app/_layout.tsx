...
const TABS = [
  ...
  { name: 'live-operations', label: 'Live', icon: 'git-network-outline', iconActive: 'git-network' },
          ...
          return (
            ...
            >
              <View style={[styles.tabIconWrap, isActive && styles.tabIconWrapActive]}>
                <Ionicons name={(isActive ? tab.iconActive : tab.icon) as any} size={20} color={isActive ? '#0ea5e9' : '#475569'} />
              </View>
...
export default function RootLayout() {
  const pathname = usePathname();
  const showTabs = [
    '/', '/positions', '/trades', '/alerts', '/settings', '/live-operations',
    '/risk-settings', '/trading-settings', '/strike-selection', '/discord-settings',
  ].includes(pathname);
...
const styles = StyleSheet.create({
  ...
  tabBarShell: { backgroundColor: '#0d1826', borderTopWidth: 1, borderTopColor: '#1e2d3d' },
  tabBar: { paddingBottom: 8, paddingTop: 6, paddingHorizontal: 4, gap: 2 },
  tabItem: { width: 62, alignItems: 'center', gap: 3 },
  tabIconWrap: { width: 36, height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center' },
  tabIconWrapActive: { backgroundColor: 'rgba(14, 165, 233, 0.12)' },
  tabLabel: { fontSize: 10, color: '#475569', fontWeight: '500' },
