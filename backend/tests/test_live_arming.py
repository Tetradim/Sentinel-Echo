...
class LiveArmingTests(unittest.TestCase):
    ...
    def test_arm_live_trading_treats_serialized_false_readiness_as_blocked(self):
        from live_arming import arm_live_trading
        ...
        with self.assertRaises(RuntimeError):
            asyncio.run(
                arm_live_trading(
                    ...
                    duration_minutes=15,
                    confirmation="ARM LIVE TRADING",
                    readiness={"ready_for_live": "false", "blocking_issues": []},
                    operator="tester",
                )
            )
    ...
    def test_blocked_arm_audit_normalizes_malformed_blocking_issues(self):
        from live_arming import arm_live_trading
        ...
        with self.assertRaises(RuntimeError):
            asyncio.run(
                arm_live_trading(
                    ...
                    duration_minutes=15,
                    confirmation="ARM LIVE TRADING",
                    readiness={"ready_for_live": False, "blocking_issues": "blocked"},
                    operator="tester",
                )
            )

        self.assertEqual(db.events[-1]["details"]["blocking_issues"], [])
